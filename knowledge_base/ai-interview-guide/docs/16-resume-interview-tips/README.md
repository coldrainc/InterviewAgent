...
```
【项目背景】
我曾经在一个 RAG 项目中选择了小模型（GPT-3.5-turbo）来节省成本，认为它足够处理简单问答。
...
**面试话术：**
> "我最失败的技术决策是在 RAG 项目中过早优化成本，用 GPT-3.5 替代 GPT-4，结果用户反馈很差，最终成本反而更高（因为换了两次）。这次失败让我建立了'质量优先，分级验证'的上线流程：先用最优模型做 baseline，确认质量可接受后再优化成本。"
