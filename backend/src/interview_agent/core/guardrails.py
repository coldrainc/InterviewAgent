...
class HarnessGuardrails:
    """Input/output guardrails for the interview harness."""
    ...
    def check_model_output(self, text: str) -> GuardrailResult:
        ...
        if len(cleaned) > self.max_output_chars:
            cleaned = cleaned[: self.max_output_chars].rstrip() + "\n\n（回答较长，已自动截断。）"
            findings.append(
                GuardrailFinding(
    ...
    def blocked_message(self, findings: list[GuardrailFinding]) -> str:
        ...
        return f"我先暂停处理这条输入：{reasons}请换一种方式回答当前问题。"
